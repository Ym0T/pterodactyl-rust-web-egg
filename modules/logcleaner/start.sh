...
mapfile -t large_logs < <(find "$LOG_DIR" -type f -name '*.log' -size +${MAX_SIZE_MB}M 2>/dev/null || true)
if (( ${#large_logs[@]} )); then
...
mapfile -t old_logs < <(find "$LOG_DIR" -type f -name '*.log' -mtime +${MAX_AGE_DAYS} 2>/dev/null || true)
if (( ${#old_logs[@]} )); then
